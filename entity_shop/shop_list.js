...
var Page = {
	data: {
		searchData: {
			...
			query_user_id: 0,
			position_id: 0,
			shop_state: 5, // 店铺状态（待开通=1，正常=2，停用=3，注销=4，全部=5，试用=6）
			source_id: 0,
			shop_version_id: 0
		}, //查询对象
		...
		post("/api/EntityShopManage/query",
			...
			function(datas) {
				if(datas.code.indexOf("success") != -1) {
					...
					//渲染--数量信息
					$(".entity_shop_footer .examine_menu em").html(datas.data.total.all_num);
					$('#storeState_5').next().find('div').html('全部(' + datas.data.total.all_num + ')');
					$('#storeState_2').next().find('div').html('正常(' + datas.data.total.normal_num + ')');
	...
	createShopListTable: function() {
		...
		for(var i = 0; i < data.length; i++) {
			...
			html += '<td>' + data[i].order_amount + '</td>';
			html += '<td class="' + audit_state_arr[data[i].finance_state].css + '">' + audit_state_arr[data[i].finance_state].name + '</td>';
			html += '<td>' + data[i].wangwang + '</td>';
			html += '<td>' + data[i].contacts + '</td>';
			html += '<td>' + data[i].contacts_mobile + '</td>';
			html += '<td>' + data[i].goods_num + '</td>';
			html += '<td>' + data[i].member_num + '</td>';
			html += '<td>' + data[i].order_num + '</td>';
			html += '<td>' + data[i].deal_amount + '</td>';
			html += '<td>' + data[i].position_name + '</td>';
			html += '<td>' + data[i].modify_user_name + '</td>';
